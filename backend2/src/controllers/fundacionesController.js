...
// ─────────────────────────────────────────────
// POST /api/fundaciones/registro
// ─────────────────────────────────────────────
export async function registroFundacion(req, res) {
  try {
    const {
      nombre, nit, tipo, direccion, localidad, correo, telefono,
      password, representante_legal,
      doc_camara_comercio, doc_rut, doc_cedula_rep, doc_estatutos
    } = req.body
    ...
    // 4. Insertar en la tabla fundaciones
    ...
    return res.status(201).json({
      mensaje: '¡Fundación registrada! Su solicitud está en revisión.',
      token,
      fundacion
    })
...
// ─────────────────────────────────────────────
// POST /api/fundaciones/login
// ─────────────────────────────────────────────
export async function loginFundacion(req, res) {
  try {
    ...
    return res.status(200).json({
      mensaje: `¡Bienvenida, ${fundacion.nombre}!`,
      token,
      fundacion: fundacionSinPassword
    })
...
// ─────────────────────────────────────────────
// GET /api/fundaciones  — lista todas
// ─────────────────────────────────────────────
export async function getFundaciones(req, res) {
  try {
    const { data, error } = await supabase
      .from('fundaciones')
...
// ─────────────────────────────────────────────
export async function getFundacion(req, res) {
  try {
    const { data, error } = await supabase
      .from('fundaciones')
      .select('id_fundaciones, nombre, nit, tipo, direccion, localidad, correo, telefono, estado, fecha_registro')
